        ...
        >
          ...
          <div className="flex items-center gap-3 mt-4 md:mt-0">
            <button className="relative w-10 h-10 rounded-xl bg-eco-surface border border-eco-border flex items-center justify-center text-eco-muted hover:text-white transition-colors">
              <Bell size={18} />
              <span className="absolute -top-1 -right-1 w-4 h-4 bg-eco-orange rounded-full text-[10px] flex items-center justify-center">3</span>
            </button>
          </div>
        </motion.div>
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        {[
          { label: 'Players', value: '24', icon: Users, color: '#97B3D2' },
          { label: 'Win Rate', value: '87%', icon: TrendingUp, color: '#B0C8E0' },
          { label: 'Next Game', value: 'Apr 5', icon: Calendar, color: '#6A9BC7' },
          { label: 'Unread', value: '3', icon: MessageCircle, color: '#4A7FB5' },
        ].map((stat) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glow-card p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xs font-mono uppercase tracking-widest text-eco-orange flex items-center gap-2">
              <Calendar size={14} />
              Upcoming
            </h3>
            {isAdmin && (
              <button 
                ...
                className="text-[10px] font-mono uppercase tracking-widest text-eco-muted hover:text-eco-orange transition-colors flex items-center gap-1 bg-eco-surface2 px-2 py-1 rounded-md border border-white/5"
              >
        ...
        {/* Teams Summary */}
        <div className="glow-card p-6">
          <h3 className="text-xs font-mono uppercase tracking-widest text-eco-orange mb-4 flex items-center gap-2">
            <Trophy size={14} />
            Teams
          </h3>
          <div className="space-y-3">
            {TEAMS.map((team) => (
              <div key={team.id} className="flex items-center gap-4 p-3 rounded-xl bg-eco-surface2 border border-eco-border">
                <div className="w-10 h-10 rounded-xl bg-eco-orange/10 flex items-center justify-center font-display text-lg text-eco-orange">
                  {team.roster.length}
                </div>
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      ...
      <div className="glow-card p-4">
        <h3 className="text-xs font-mono uppercase tracking-widest text-eco-orange mb-4 px-2">Channels</h3>
        {['general', '2011-boys', '2012-girls', ...(isAdmin ? ['payments', 'coaches'] : [])].map((channel) => {
          ...
          return (
            ...
            >
              <span className="text-sm font-heading text-white capitalize">
                # {channel.replace('-', ' ')}
              </span>
              {unread > 0 && (
                <span className="w-5 h-5 rounded-full bg-eco-orange text-[10px] flex items-center justify-center text-white font-bold">
                  {unread}
                </span>
              )}
            </button>
      ...
      <div className="lg:col-span-2 glow-card flex flex-col" style={{ minHeight: '500px' }}>
        <div className="p-4 border-b border-eco-border">
          <h3 className="font-heading font-bold text-white"># General</h3>
          <p className="text-xs text-eco-muted">Team-wide announcements and chat</p>
        </div>

        <div className="flex-1 p-4 space-y-4 overflow-y-auto">
          {MESSAGES.filter((m) => m.channel === 'general').map((msg) => (
            <div key={msg.id} className="flex gap-3">
              <div className="w-8 h-8 rounded-full bg-eco-surface2 border border-eco-border flex items-center justify-center flex-shrink-0">
                <span className="text-xs font-bold text-eco-orange">{msg.sender[0]}</span>
              </div>
              <div>
        ...
        <div className="p-4 border-t border-eco-border">
          <div className="flex gap-2">
            ...
            />
            <button className="btn-glow !px-4 !py-2.5">
              <Send size={16} />
            </button>
          </div>
        </div>
    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="stat-card">
          ...
          <p className="font-display text-3xl text-eco-muted-light">${totalOverdue.toLocaleString()}</p>
        </div>
      </div>
      ...
      <div className="glow-card overflow-hidden">
        <div className="p-4 border-b border-eco-border flex items-center justify-between">
          ...
          <div className="relative">
            ...
            <input className="input-field !py-1.5 !pl-9 !pr-3 text-xs w-48" placeholder="Search..." />
          </div>
        </div>
          ...
          {PAYMENTS.map((payment) => (
            <div key={payment.id} className="flex items-center gap-4 px-4 py-3 hover:bg-eco-surface2/50 transition-colors">
              <div className="w-8 h-8 rounded-lg bg-eco-surface2 border border-eco-border flex items-center justify-center flex-shrink-0">
                <span className="text-xs font-bold text-eco-orange">{payment.player[0]}</span>
              </div>
              <div className="flex-1 min-w-0">
              ...
              <div className="text-right">
                ...
                <span className={`inline-block text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full font-bold ${
                  payment.status === 'paid'
                    ...
                    : 'bg-eco-muted/20 text-eco-muted-light'
                }`}>
                  {payment.status}
                </span>
              </div>
              ...
              <form onSubmit={handleSaveKey} className="space-y-3">
                <div className="flex flex-col sm:flex-row gap-3 items-end">
                  ...
                  <div className="flex gap-2 w-full sm:w-auto">
                    <button
                      type="submit"
                      className="flex-1 sm:flex-none px-4 py-2 bg-eco-orange text-white rounded-xl text-xs font-heading font-semibold hover:bg-eco-orange/90 transition-all shadow-glow-sm"
                    >
                      Save Key
                    </button>
                    {apiKey && (
      ...
      >
          ...
          <div key={i} className={`flex gap-3 ${msg.role === 'user' ? 'flex-row-reverse' : ''}`}>
            ...
            <div className={`max-w-[80%] rounded-2xl px-5 py-3 whitespace-pre-line ${
              msg.role === 'model'
                ...
                : 'bg-eco-orange/20 border border-eco-orange/30'
            }`}>
        ...
        {isLoading && (
          <div className="flex gap-3">
            <div className="w-8 h-8 rounded-full bg-eco-orange/20 text-eco-orange flex items-center justify-center flex-shrink-0 animate-pulse">
              <Bot size={14} />
            </div>
            <div className="bg-eco-surface2 border border-eco-border rounded-2xl px-5 py-3 flex items-center gap-2">
              <Loader2 className="animate-spin text-eco-orange" size={14} />
        ...
        <div className="flex gap-2 mt-3 overflow-x-auto scrollbar-hide">
          {[
            ...
            <button
              ...
              onClick={() => setInput(q)}
              disabled={isLoading}
              className="text-[11px] px-3.5 py-1.5 rounded-full bg-eco-surface border border-eco-border text-eco-muted-light hover:text-white hover:border-eco-orange/30 transition-all whitespace-nowrap disabled:opacity-50"
            >
              {q}
            </button>
          ))}
...
function AnalyticsTab() {
  ...
  const handleUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      setTimeout(() => {
        ...
        alert('Scoresheet successfully processed. Analytics engine updated!')
      }, 2000)
    }
  }
  ...
  return (
    <div className="space-y-8">
      {/* Upload Section */}
      <div className="glow-card p-6 md:p-8 flex flex-col items-center justify-center text-center border-dashed border-2 border-eco-orange/30 group hover:border-eco-orange/60 transition-colors">
        <UploadCloud size={48} className="text-eco-orange mb-4 group-hover:-translate-y-2 transition-transform duration-300" />
        <h3 className="font-display text-2xl text-white mb-2">Upload Scoresheet</h3>
        <p className="text-eco-muted-light text-sm max-w-md mx-auto mb-6">
          Upload game box scores, advanced tracked stats spreadsheets, or video breakdown CSVs here. The EcoHoops analytics engine will automatically process the data and update player profiles and team metrics.
        </p>
        
        <label className="btn-glow cursor-pointer inline-flex items-center gap-2">
          {isUploading ? 'Processing File...' : 'Select File'}
          <input type="file" className="hidden" accept=".csv, .pdf, .xlsx" onChange={handleUpload} disabled={isUploading} />
        </label>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="glow-card p-6">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between text-xs text-white mb-1"><label>Offensive Rating (Points per 100 pos)</label> <span>114.2</span></div>
              <div className="w-full h-2 rounded-full bg-eco-surface2 overflow-hidden"><div className="w-[85%] h-full bg-eco-orange rounded-full"></div></div>
            </div>
            <div>
              <div className="flex justify-between text-xs text-white mb-1"><label>Defensive Rating (Points allowed per 100 pos)</label> <span>98.6</span></div>
              <div className="w-full h-2 rounded-full bg-eco-surface2 overflow-hidden"><div className="w-[92%] h-full bg-eco-blue rounded-full"></div></div>
            </div>
            <div>
              <div className="flex justify-between text-xs text-white mb-1"><label>Effective Field Goal % (eFG%)</label> <span>53.4%</span></div>
              <div className="w-full h-2 rounded-full bg-eco-surface2 overflow-hidden"><div className="w-[60%] h-full bg-eco-muted-light rounded-full"></div></div>
            </div>
          </div>
        </div>
            ...
            {[
              ...
              <div key={i} className="flex justify-between items-center p-3 rounded-xl bg-eco-surface2 border border-eco-border text-sm">
                ...
                <div className="flex flex-col items-end flex-shrink-0">
                  <span className={`text-[10px] uppercase font-bold tracking-wider ${log.status === 'Processed' ? 'text-eco-blue' : 'text-red-500'}`}>{log.status}</span>
