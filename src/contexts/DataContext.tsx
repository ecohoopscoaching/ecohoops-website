...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [teams, setTeams] = useState<Team[]>(() => {
    const saved = localStorage.getItem('ecohoops_teams_v6')
  ...
  useEffect(() => {
    const cleanFlagKey = 'ecohoops_clean_baseline_v15'
    if (!localStorage.getItem(cleanFlagKey)) {
      localStorage.setItem('ecohoops_teams_v6', JSON.stringify(TEAMS))
