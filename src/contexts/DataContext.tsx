...
export function DataProvider({ children }: { children: ReactNode }) {
  ...
  const [teams, setTeams] = useState<Team[]>(() => {
    const saved = localStorage.getItem('ecohoops_teams_v3')
  ...
  useEffect(() => {
    const cleanFlagKey = 'ecohoops_clean_baseline_v11'
    if (!localStorage.getItem(cleanFlagKey)) {
      localStorage.setItem('ecohoops_teams_v3', JSON.stringify(TEAMS))
